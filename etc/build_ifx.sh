...
then
	...
	cd ../../libpvn/src
	make COMPILER=icx NATIVE=haswell MARCH=${TGT} NDEBUG=3 SAFE=SV2,NRM OPENMP=0 clean
	make COMPILER=icx NATIVE=haswell MARCH=${TGT} NDEBUG=3 SAFE=SV2,NRM OPENMP=0 -j
	cd ../../KogAcc/src
	make LAPACK=sequential clean all
	cd ../etc
fi
