...
SUBROUTINE ZKSVD1(JOB, M, B, G, LDG, U, LDU, V, LDV, SV, LDB, W, NW, D, ND, O, NO, INFO)
  ...
  ! END INTERFACE
  ! INTERFACE
  !    SUBROUTINE ZBKSVD0(JS, B2, NB, GB, UB, VB, LDB, SB, WB, LW, OD, OB, O, INFO)
  !      USE, INTRINSIC :: ISO_FORTRAN_ENV, ONLY: REAL64
  !      IMPLICIT NONE
  !      INTEGER, INTENT(IN) :: JS, B2, NB, LDB, LW, OB(2,*)
  !      COMPLEX(KIND=REAL64), INTENT(INOUT) :: GB(LDB,B2,NB)
  !      COMPLEX(KIND=REAL64), INTENT(OUT) :: UB(LDB,B2,NB), VB(LDB,B2,NB)
  !      REAL(KIND=REAL64), INTENT(OUT) :: SB(B2,NB), WB(LW,NB)
  !      INTEGER, INTENT(OUT) :: OD(2,B2,NB)
  !      INTEGER, INTENT(INOUT) :: O(2,NB), INFO
  !    END SUBROUTINE ZBKSVD0
  ! END INTERFACE
  ! INTERFACE
  ...
  EXTERNAL :: ZBPACK, ZBKSVDD, ZBKSVD0, ZBUNPACK, ZBUPDATE
#ifdef ANIMATE
  TYPE(c_ptr) :: CTX
