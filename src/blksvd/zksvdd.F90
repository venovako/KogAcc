...
#define CR_HYPOT HYPOT
  ...
  DO STP = 0, MRQSTP-1
     ...
     M = 0
!$OMP PARALLEL DO DEFAULT(NONE) SHARED(G,U,W,O,N,LDG,LDU,I,LUACC) PRIVATE(G2,U2,V2,P,Q,WV,WS,T,L,ES) REDUCTION(+:M) IF(LOMP)
     DO J = 1, I
        L = (N * (N - 1)) / 2
        P = O(1,L+J)
        Q = O(2,L+J)
...
!$OMP END PARALLEL DO
     IF (M .NE. 0) THEN
        ...
        RETURN
     END IF
!$OMP PARALLEL DO DEFAULT(NONE) SHARED(G,V,W,O,N,LDG,LDV,I,LVACC) PRIVATE(V2,P,Q,WV,WS,T,L) REDUCTION(+:M) IF(LOMP)
     DO J = 1, I
        L = (N * (N - 1)) / 2
        P = O(1,L+J)
        Q = O(2,L+J)
