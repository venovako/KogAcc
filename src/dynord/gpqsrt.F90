  ...
  DO WHILE (L .LT. N)
     ...
     L = L * 2
     M = 0
     !$OMP PARALLEL DO DEFAULT(NONE) SHARED(AW,AP,AQ,BW,BP,BQ,K,L,N,FLIP) PRIVATE(J,X,Y) REDUCTION(+:INFO,M) IF(Z .NE. 0)
     DO I = 1, N, L
        X = MIN(K, N-I+1)
        IF (X .LT. K) THEN
           J = N
  ...
  END DO
  IF (FLIP) THEN
     !$OMP PARALLEL DO DEFAULT(NONE) SHARED(AW,AP,AQ,BW,BP,BQ,N) IF(Z .NE. 0)
     DO I = 1, N
        AW(I) = BW(I)
        AP(I) = BP(I)
        AQ(I) = BQ(I)
