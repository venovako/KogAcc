!>@brief \b CKSVD1X tests the CKSVD1 routine.
PROGRAM CKSVD1X
#ifdef ANIMATE
  ...
  INTERFACE
     SUBROUTINE CKSVD1(JOB, M, B, G, LDG, U, LDU, V, LDV, SV, LDB, W, NW, D, ND, O, NO, INFO)
       ...
       COMPLEX(KIND=REAL32), INTENT(INOUT) :: G(LDG,M), U(LDU,M), V(LDV,M)
#ifdef ANIMATE
       REAL(KIND=REAL32), INTENT(INOUT), TARGET :: SV(M)
#else
       REAL(KIND=REAL32), INTENT(OUT) :: SV(M)
#endif
       COMPLEX(KIND=REAL32), INTENT(OUT) :: W(NW) ! HACK
  ...
  ! complex for alignment purposes, in reality mixed real and complex
  COMPLEX(KIND=K), ALLOCATABLE :: W(:)
...
#define VIS_START PVN_CVIS_START_F
#define VIS_STOP PVN_CVIS_STOP_F
#endif
#define BRDG CBRDG
#define RDINP CRDINP
#define WROUT CWROUT
