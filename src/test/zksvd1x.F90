!>@brief \b ZKSVD1X tests the ZKSVD1 routine.
PROGRAM ZKSVD1X
#ifdef ANIMATE
  USE, INTRINSIC :: ISO_C_BINDING
#endif
  USE, INTRINSIC :: ISO_FORTRAN_ENV, ONLY: INT64, REAL64, REAL128, ERROR_UNIT, OUTPUT_UNIT
  !$ USE OMP_LIB
  IMPLICIT NONE
  INTERFACE
     PURE SUBROUTINE IBDIMS(N, B, M, M_B, NW, ND, NO, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(INOUT) :: N, B, M, INFO
       INTEGER, INTENT(OUT) :: M_B, NW, ND, NO
     END SUBROUTINE IBDIMS
  END INTERFACE
  INTERFACE
     SUBROUTINE ZBRDG(M, N, G, LDG, INFO)
       USE, INTRINSIC :: ISO_FORTRAN_ENV, ONLY: REAL64
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: M, N, LDG
       COMPLEX(KIND=REAL64), INTENT(OUT) :: G(LDG,M)
       INTEGER, INTENT(INOUT) :: INFO
     END SUBROUTINE ZBRDG
  END INTERFACE
  INTERFACE
     PURE SUBROUTINE INITRC(N, O, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       INTEGER, INTENT(OUT) :: O(2,*), INFO
     END SUBROUTINE INITRC
  END INTERFACE
  INTERFACE
     PURE SUBROUTINE INITCC(N, O, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       INTEGER, INTENT(OUT) :: O(2,*), INFO
     END SUBROUTINE INITCC
  END INTERFACE
  INTERFACE
     SUBROUTINE INITME(N, O, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       INTEGER, INTENT(OUT) :: O(2,*), INFO
     END SUBROUTINE INITME
  END INTERFACE
  INTERFACE
     SUBROUTINE INITMM(N, O, INFO)
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       INTEGER, INTENT(OUT) :: O(2,*), INFO
     END SUBROUTINE INITMM
  END INTERFACE
  INTERFACE
     SUBROUTINE ZKSVD1(JOB, M, B, G, LDG, U, LDU, V, LDV, SV, LDB, W, NW, D, ND, O, NO, INFO)
       USE, INTRINSIC :: ISO_FORTRAN_ENV, ONLY: REAL64, REAL128
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: JOB, M, B, LDG, LDU, LDV, LDB, NW, ND, NO
       COMPLEX(KIND=REAL64), INTENT(INOUT) :: G(LDG,M), U(LDU,M), V(LDV,M)
#ifdef ANIMATE
       REAL(KIND=REAL64), INTENT(INOUT), TARGET :: SV(M)
#else
       REAL(KIND=REAL64), INTENT(OUT) :: SV(M)
#endif
       COMPLEX(KIND=REAL64), INTENT(OUT) :: W(NW) ! HACK
       REAL(KIND=REAL128), INTENT(OUT) :: D(ND)
       INTEGER, INTENT(INOUT) :: O(2,NO), INFO
     END SUBROUTINE ZKSVD1
  END INTERFACE

  INTEGER, PARAMETER :: K = REAL64, KK = REAL128, CLAL = 256
  CHARACTER(LEN=CLAL) :: BN
  INTEGER(KIND=INT64) :: C0, C1, CR
  REAL(KIND=REAL128) :: T
  INTEGER :: JOB, M, N, B, M_B, LDG, LDU, LDV, LDB, NW, ND, NO, INFO, I, J, L
  COMPLEX(KIND=K), ALLOCATABLE :: G(:,:)
  !DIR$ ATTRIBUTES ALIGN: 64:: G
  COMPLEX(KIND=K), ALLOCATABLE :: U(:,:)
  !DIR$ ATTRIBUTES ALIGN: 64:: U
  COMPLEX(KIND=K), ALLOCATABLE :: V(:,:)
  !DIR$ ATTRIBUTES ALIGN: 64:: V
#ifdef ANIMATE
  REAL(KIND=K), ALLOCATABLE, TARGET :: SV(:)
#else
  REAL(KIND=K), ALLOCATABLE :: SV(:)
#endif
  !DIR$ ATTRIBUTES ALIGN: 64:: SV
  ! complex for alignment purposes, in reality mixed real and complex
  COMPLEX(KIND=K), ALLOCATABLE :: W(:)
  !DIR$ ATTRIBUTES ALIGN: 64:: W
  REAL(KIND=KK), ALLOCATABLE :: D(:)
  !DIR$ ATTRIBUTES ALIGN: 64:: D
  INTEGER, ALLOCATABLE :: O(:,:)
  !DIR$ ATTRIBUTES ALIGN: 64:: O
#ifdef ANIMATE
  CHARACTER(LEN=CLAL) :: CN
  TYPE(c_ptr) :: CTX
  TYPE(c_ptr), POINTER :: CP
  TYPE(c_ptr), EXTERNAL :: PVN_CVIS_START
  INTEGER(KIND=c_int), EXTERNAL :: PVN_CVIS_STOP
#define VIS_START PVN_CVIS_START
#define VIS_STOP PVN_CVIS_STOP
#endif
#define BRDG ZBRDG
#define RDINP ZRDINP
#define WROUT ZWROUT
#define KSVD1 ZKSVD1
#include "hksvd1x.F90"
9 FORMAT(3(ES25.17E3,A))
CONTAINS
#include "zrdinp.F90"
#include "zwrout.F90"
END PROGRAM ZKSVD1X
