...
PROGRAM WKSVDDX
  ...
  INTERFACE
     SUBROUTINE WKSVDD(JOB, N, G, LDG, U, LDU, V, LDV, SV, W, D, O, INFO)
       ...
       REAL(KIND=REAL64), INTENT(INOUT) :: SV(N), W(*)
  ...
  INFO = -INT(W(4))
  !$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(I) SHARED(N,D,SV,INFO)
  DO I = 1, N
#ifdef __GFORTRAN__
     D(I) = TRANSFER(SCALE(REAL(SV(I), REAL128), INFO), D(I))
#else
  ...
  INFO = L
  CALL ZWROUT(BN, J, N, U, LDU, V, LDV, D, INFO)
